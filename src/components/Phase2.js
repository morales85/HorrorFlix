...
console.log(movies)
    return (
        ...
        {movies.some(m => m.phase === 2 && m.isRented) ?
            <div>
              <h2>Rented: </h2>
                {movies.filter(m => m.phase === 2 && m.isRented && m.title.toLowerCase().includes(input)).map(m => <Movie key={m.id} movie={m} rentMovie={this.props.rentMovie} />)}
            </div> : ""
          }
                <h2>Phase 2:</h2>
                {movies.filter(m => m.phase === 2 && !m.isRented && m.title.toLowerCase().includes(input)).map(m => <Movie key={m.id} movie={m} rentMovie={this.props.rentMovie} />)}
